...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!(
            "Usage: cargo run --release --example overlay_bench_diff -- <baseline.csv> <current.csv> [max_regression_percent] [--op-threshold op=pct ...] [--case-threshold case=pct ...] [--case-op-threshold case:op=pct ...]"
        );
        std::process::exit(2);
    }
...
fn parse_threshold_args(
    ...
    while i < args.len() {
        ...
        // Positional threshold already consumed from args[3].
        i += 1;
    }
