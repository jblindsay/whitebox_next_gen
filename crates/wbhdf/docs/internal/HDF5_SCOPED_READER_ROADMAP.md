...
- [x] Record go/no-go decision with blockers and next actions.
  ...
  - Supported-layout matrix, rollback runbook, reference-tolerance matrix, and MODIS scope boundary are now documented, and repeatable non-HDF regression confidence is now refreshed with a passing smoke matrix; real multi-level tree validation depth is improved with ATL08 canopy/terrain plus VNP13 + VNP21 science/geolocation (LST/latitude/longitude) fixture probes, but broader generalized end-to-end confidence gaps remain.
  Next actions:
...
Follow-up real-fixture multilevel evidence (2026-06-01):
...
- Added fixture-backed integration test
  `viirs_vnp21_longitude_bounded_chunk_index_probe_returns_expected_chunk_records` that verifies a
  non-leaf chunk-index root for VNP21 geolocation longitude, confirms bounded non-origin record
  retrieval, and validates direct chunk-payload decode viability/in-range geolocation values.
- Added fixture-backed integration test
  `viirs_vnp13_ndvi_bounded_chunk_index_probe_returns_expected_chunk_records` that verifies a
  non-leaf chunk-index root for VNP13 NDVI, confirms bounded record retrieval, and validates
  direct chunk-payload decode viability with reference-like NDVI values.
- Added fixture-backed integration test
...
- Added fixture-backed integration test
  ...
  finite slope values.
- Added fixture-backed integration test
  `atl08_h_canopy_20m_bounded_chunk_index_probe_returns_records` that verifies bounded
...
- Added fixture-backed integration test
  ...
  `{10000,5}` chunk-shape path.
- Added fixture-backed integration test
  `atl08_canopy_h_metrics_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy metrics
  (`canopy_h_metrics`) and validates direct chunk-payload decode viability for a wider 2D
  `{10000,18}` chunk-shape path.
- Added fixture-backed integration test
  `atl08_subset_can_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy categorical
  flags (`subset_can_flag`), and validates direct chunk-payload decode viability on a
  shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_subset_te_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain categorical
  flags (`subset_te_flag`), and validates direct chunk-payload decode viability on a
  shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_te_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain quality
  scores (`te_quality_score`), and validates direct chunk-payload decode viability on a
  shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_can_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy quality
  scores (`can_quality_score`), and validates direct chunk-payload decode viability on a
  shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt2l_te_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain quality
  scores on a second beam (`/gt2l/land_segments/terrain/te_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt2l_can_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy quality
  scores on a second beam (`/gt2l/land_segments/canopy/can_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt2l_subset_te_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain categorical
...
- Added fixture-backed integration test
  ...
  chunk-payload decode viability on a shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_gt1r_te_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain quality
  scores on a right beam (`/gt1r/land_segments/terrain/te_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt1r_can_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy quality
  scores on a right beam (`/gt1r/land_segments/canopy/can_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt1r_subset_te_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain categorical
  flags on a right beam (`/gt1r/land_segments/terrain/subset_te_flag`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_gt1r_subset_can_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy categorical
  flags on a right beam (`/gt1r/land_segments/canopy/subset_can_flag`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_gt2r_te_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain quality
  scores on a right beam (`/gt2r/land_segments/terrain/te_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt2r_can_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy quality
  scores on a right beam (`/gt2r/land_segments/canopy/can_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt2r_subset_te_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain categorical
  flags on a right beam (`/gt2r/land_segments/terrain/subset_te_flag`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_gt2r_subset_can_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy categorical
  flags on a right beam (`/gt2r/land_segments/canopy/subset_can_flag`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_gt3l_te_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain quality
  scores on a third beam (`/gt3l/land_segments/terrain/te_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt3l_can_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy quality
  scores on a third beam (`/gt3l/land_segments/canopy/can_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt3l_subset_te_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain categorical
  flags on a third beam (`/gt3l/land_segments/terrain/subset_te_flag`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_gt3l_subset_can_flag_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy categorical
  flags on a third beam (`/gt3l/land_segments/canopy/subset_can_flag`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 2D `{10000,5}` byte-category path.
- Added fixture-backed integration test
  `atl08_gt1l_te_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 terrain quality
  scores on a left beam (`/gt1l/land_segments/terrain/te_quality_score`), validating direct
  chunk-payload decode viability on a shuffle+deflate-filtered 1D `{10000}` byte-quality path.
- Added fixture-backed integration test
  `atl08_gt1l_can_quality_score_bounded_chunk_index_probe_returns_records` that verifies bounded
  chunk-index traversal parity with direct leaf-chain enumeration for ATL08 canopy quality
...
- Targeted confirmations passed:
  ...
  - `cargo test -p wbhdf atl08_gt3l_subset_te_flag_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt3l_can_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt3l_te_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt2r_subset_can_flag_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt2r_subset_te_flag_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt2r_can_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt2r_te_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt1r_subset_can_flag_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt1r_subset_te_flag_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt1r_can_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_gt1r_te_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  ...
  - `cargo test -p wbhdf atl08_can_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_te_quality_score_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_subset_te_flag_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_subset_can_flag_bounded_chunk_index_probe_returns_records -- --nocapture`
  - `cargo test -p wbhdf atl08_canopy_h_metrics_bounded_chunk_index_probe_returns_records -- --nocapture`
  ...
  - `cargo test -p wbhdf viirs_vnp21_longitude_bounded_chunk_index_probe_returns_expected_chunk_records -- --nocapture`
