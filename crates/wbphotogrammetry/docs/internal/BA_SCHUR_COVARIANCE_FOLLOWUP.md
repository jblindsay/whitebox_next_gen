...
- [ ] Add block indexing and sparse normal-equation accumulation in alignment BA internals.
- [ ] Implement Schur reduction and reduced camera system solve.
- [ ] Back-substitute structure updates from solved camera increments.
- [ ] Integrate LM damping and robust weights into reduced solve path.
- [ ] Add stable fallback trigger to current non-Schur path for weak/degenerate systems.
- [ ] Emit covariance diagnostics in alignment stats (with explicit field semantics).
- [ ] Update README limitation wording and capability notes.
...
## Definition of Done
1. Schur BA path is enabled as a stable production path (with guarded fallback).
2. Camera covariance diagnostics are emitted and documented.
3. Regression suite remains green and benchmark results are recorded.
