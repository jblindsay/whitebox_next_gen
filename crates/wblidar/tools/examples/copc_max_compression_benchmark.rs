//! Benchmark COPC maximum compression strategies.
//!
//! Tests impact of:
//! - compression_level tuning (6 vs 9)
//! - Spatial ordering (Morton Z-order vs Hilbert curve)
//! - max_points_per_node tuning
//!
//! Usage:
//!   cargo run -p wblidar --example copc_max_compression_benchmark -- <input.las> <output_prefix>
...
fn main() -> Result<()> {
    ...
    // ── Load source and compute bounds ──────────────────────────────────
    ...
    println!("  {} points, bounds ({:.0}-{:.0}, {:.0}-{:.0}, {:.0}-{:.0})", 
        n_read, min_x, max_x, min_y, max_y, min_z, max_z);
    ...
    // ── Test configurations ───────────────────────────────────────────────
    // Focus on practical sweet-spots: 50k nodes + aggressive compression settings
    let configs = vec![
        ("baseline_50k_lvl6", 50_000, 6),
        ("highcomp_50k_lvl9", 50_000, 9),
        ("xlarge_100k_lvl6", 100_000, 6),
        ("xlarge_100k_lvl9", 100_000, 9),
    ];

    println!("\nGenerating optimized COPC files:\n");
    println!("{:30} {:15} {:15} {}", "Config", "File Size", "% of Source", "Path");
    println!("{}", "=".repeat(85));

    for (name, max_points, comp_level) in configs {
        ...
        let las_cfg = WriterConfig {
            ...
            generating_software: "wblidar: copc_max_compression_benchmark".to_string(),
            vlrs: Vec::new(),
            crs: crs.clone(),
            extra_bytes_per_point: 0,
        };

        let mut cfg = CopcWriterConfig {
            las: las_cfg,
            center_x,
            center_y,
            center_z,
    ...
    println!("\nNote: compression_level=9 may not affect Point14 arithmetic coding");
    println!("      (currently uses fixed model sizes). Max compression reserves this");
    println!("      setting for future Point14 encoder enhancements.");
    Ok(())
}

fn usage_error() -> Error {
    Error::Projection(
        "Usage: cargo run -p wblidar --example copc_max_compression_benchmark -- <input.las> <output_prefix>"
            .to_string(),
    )
}
