...
- Fixed backend installation failure on Windows when QGIS is installed via the OSGeo4W installer. The OSGeo4W bundled Python does not include `pip` by default. The install command generated by the plugin dialog now bootstraps `pip` automatically via `ensurepip` when it is not present, so the one-click installation path works on OSGeo4W without any manual pre-configuration. Updated installation troubleshooting documentation with OSGeo4W-specific guidance.
