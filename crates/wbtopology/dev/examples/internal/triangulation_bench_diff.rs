...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!(
            "Usage: cargo run --release --example triangulation_bench_diff -- <baseline.csv> <current.csv> [max_regression_percent] [--op-threshold op=pct ...] [--case-threshold case=pct ...] [--case-op-threshold case:op=pct ...]"
        );
        std::process::exit(2);
    }
...
fn parse_csv(path: &str) -> BTreeMap<(String, String), BenchRow> {
    ...
    for (i, raw) in txt.lines().enumerate() {
        ...
        let (case_name, operation, iters, avg_us) = if new_schema {
            ...
            let operation = "triangulation".to_string();
            let avg_us = cols[5].trim().parse::<f64>().unwrap_or_else(|_| {
