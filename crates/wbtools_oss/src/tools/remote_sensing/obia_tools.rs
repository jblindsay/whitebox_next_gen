...
impl Tool for SegmentSlicSuperpixelsTool {
    fn metadata(&self) -> ToolMetadata {
        ToolMetadata {
            ...
            summary: r#"SLIC (Simple Linear Iterative Clustering) implements adaptive superpixel segmentation through iterative k-means clustering in CIELAB color space combined with spatial proximity constraints. Seed clusters are initialized on a regular grid across the multi-band imagery, then refined through iterative assignment and centroid updates that balance spectral similarity with spatial adjacency, enforcing connectivity constraints to produce compact, boundary-respecting segments. Key features include uniform superpixel region generation ideal for hierarchical OBIA workflows, parameter-controlled balance between compactness and spectral homogeneity, automatic spectral boundary detection, and controllable segment size and regularity enabling robust foundation segmentation. Use cases span multi-scale object extraction (buildings, vegetation patches, impervious surfaces), preprocessing for classification feature extraction, hierarchical segmentation pyramids for change detection, detailed land-cover mapping in high-resolution imagery, and scene understanding in urban monitoring and precision agriculture applications. Output segments represent homogeneous spectral-spatial regions with stable internal statistics; segment IDs index into derived feature tables; boundaries align with strong spectral transitions; segment size varies with spectral heterogeneity but remains relatively uniform within homogeneous areas; segments are immediately suitable for downstream feature extraction and classification workflows."#,
...
impl Tool for SegmentGraphFelzenszwalbTool {
    fn metadata(&self) -> ToolMetadata {
        ToolMetadata {
            ...
            summary: r#"Graph-based segmentation implements efficient connected-component analysis with spectral clustering principles. Pixels are represented as graph vertices with edges weighted by spectral distance between adjacent pixels; components emerge through progressive merging of similar neighboring pixels using union-find data structures, with global and local thresholds controlling merge decisions and preventing over-segmentation. Key features include computationally efficient connected-component approaches suitable for large-scale imagery, hierarchical segmentation reflecting multi-scale spectral variation, threshold parameters enabling sensitivity tuning across spectral gradients, direct applicability to multispectral and hyperspectral data, and flexible edge-weight definitions allowing custom spectral distance metrics. Use cases encompass large-area land-cover classification and landscape analysis, water body delineation and hydrological feature extraction, urban infrastructure mapping and sprawl monitoring, agricultural field boundary detection, forest stand delineation and structural analysis, and rapid reconnaissance segmentation workflows requiring computational efficiency. Output segment IDs correspond to connected components in the spectral graph; segments respect strong spectral boundaries while merging spectrally similar regions; segment geometry reflects both spectral and spatial coherence; output is well-suited for rapid statistical summarization and classification; large segments indicate spectrally homogeneous regions; fragmented small segments suggest high spectral heterogeneity or texture variation."#,
            category: ToolCategory::Raster,
            license_tier: LicenseTier::Open,
            params: vec![
                ToolParamSpec { name: "inputs", description: "Array of single-band input rasters.", required: true },
...
impl Tool for ClassifyObjectsEnsembleProTool {
    fn metadata(&self) -> ToolMetadata {
        ToolMetadata {
            ...
            summary: "Runs an ensemble-style object classification configuration tuned for higher stability across heterogeneous scenes.",
            category: ToolCategory::Raster,
            license_tier: LicenseTier::Open,
            params: ClassifyObjectsRandomForestTool.metadata().params,
        }
